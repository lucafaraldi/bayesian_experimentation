#!/usr/bin/env python
"""
Benchmark ENHANCED pre-trained RL policies against qLogEI and Random Search
on the COCO bbob-constrained suite.

This version works with policies trained by train_rl_enhanced.py which have:
- Enhanced state representation (16 summary + 3 surrogate + 2*dim)
- Constraint statistics (c_mean, c_std, c_min, last_c, best_c)
- Least infeasible point tracking
"""
...
MODEL_PATTERN = "coco_policy_enhanced_dim{dim}.pt"

OUT_CSV = Path("results_coco_rl_enhanced_vs_qlogei.csv")
OUT_CSV_SUMMARY = Path("results_coco_rl_enhanced_vs_qlogei_summary.csv")
...
class EnhancedRLOptimizer:
    """
    Enhanced RL optimizer with improved state representation.
    """
...
def run_benchmarks():
    ...
    for dim in DIMENSIONS:
        ...
        if rl_ckpt_path.exists():
            rl_available = True
            rl_optimizer_template = EnhancedRLOptimizer(
                dim=dim,
                ckpt_path=rl_ckpt_path,
            )
            print(f"[INFO] Loaded ENHANCED RL model: {rl_ckpt_path}")
        else:
            rl_available = False
            print(f"[WARN] ENHANCED RL model not found for dim={dim}: {rl_ckpt_path}")
        ...
        for fid in FUNCTIONS:
            for inst in INSTANCES:
                ...
                for rep in range(REPETITIONS):
                    ...
                    # --- Enhanced RL (if available) ---
                    if rl_available:
                        rl_opt = EnhancedRLOptimizer(
                            dim=dim,
                            ckpt_path=rl_ckpt_path,
                        )
                    ...
                    if rl_available:
                        print(
                            ...
                            f"Random={rs_final:.3e}, qLogEI={q_final:.3e}, RL_Enhanced={rl_final:.3e}"
                        )
                    else:
                        print(
                            f"    Summary (dim={dim}, F{fid}, inst={inst}, rep={rep+1}): "
                            f"Random={rs_final:.3e}, qLogEI={q_final:.3e}, RL_Enhanced=N/A"
                        )
